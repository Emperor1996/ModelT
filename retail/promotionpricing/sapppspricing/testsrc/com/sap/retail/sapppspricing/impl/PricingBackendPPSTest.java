...
public class PricingBackendPPSTest
{
	...
	@Mock
	private Cache<PriceInformation, ProductModel> catalogPriceCacheMock;
	@Mock
	...
	@Test
	public void testSetGetCatalogPriceCache()
	{
		assertNull(cut.getCatalogPriceCache());
		cut.setCatalogPriceCache(catalogPriceCacheMock);
		assertSame(catalogPriceCacheMock, cut.getCatalogPriceCache());
	}
	...
	public void testReadPriceInformationForProductsPriceInfoNotNull()
	{
		...
		Mockito.when(catalogPriceCacheMock.read(product)).thenReturn(pinfo);
	...
	{
		final BigDecimal calculatedPrice = new BigDecimal(99999.99);
		...
		Mockito.when(catalogPriceCacheMock.read(product)).thenReturn(null);
		...
		final ArgRecorderAnswer<Void> answerWriteCache = new ArgRecorderAnswer<Void>();
		Mockito.doAnswer(answerWriteCache).when(catalogPriceCacheMock)
				.write(Mockito.eq(product), Mockito.any(PriceInformation.class));

		final List<PriceInformation> result = cut.readPriceInformationForProducts(productModels, true);

		assertEquals(1, result.size());
		assertEquals(priceValue, result.get(0).getPriceValue());
		assertEquals(0, answerWriteCache.getArgCount());
	}
	...
	public void testReadPriceInformationForProductsPriceInfoNullExtendedAmountSet()
	{
		...
		final PriceInformation priceInfoCache = answerWriteCache.getArg(1);

		assertSame(product, answerWriteCache.getArg(0));
		assertEquals(pinfo.getPriceValue(), priceInfoCache.getPriceValue());
	}
